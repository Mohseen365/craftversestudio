...
export function CapacityPanel() {
  ...
  async function load() {
    ...
    const farthest = fetchedRows.reduce((max, row) => (row.date > max ? row.date : max), "");
    setMaxDate(farthest);
    // Initialize pagination window: start with today + 7 days or farthest if smaller
    const today = new Date().toISOString().split("T")[0];
    const initialEnd = addDays(today, 7);
    setWindowEnd(initialEnd > farthest ? farthest : initialEnd);
    setRows(fetchedRows);
  }
  ...
  return (
    <div className="space-y-6">
      <div className="overflow-x-auto rounded-lg border border-stone-200 bg-white">
                  ...
                  {expandedDate === row.date && (
                    <div className="mt-2 flex items-center gap-2">
                      ...
                      <button
                        ...
                        onClick={async () => {
                          const input = document.getElementById(`capacity-input-${row.date}`) as HTMLInputElement;
                          const newCap = parseFloat(input.value);
                          if (isNaN(newCap) || newCap < 0) return;
                          await fetch('/api/admin/capacity/override', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ date: row.date, capacity: newCap }),
                          });
                          // Refresh data
                          load();
                        }}
        ...
        {rows.length > 0 && new Date(windowEnd) < new Date(maxDate) && (
          <tfoot>
            <tr>
                  ...
                  <td className="py-3 pr-4">
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        onClick={async () => {
                          const input = document.getElementById(`progress-input-${order.id}`) as HTMLInputElement;
                          const completed = parseFloat(input.value);
                          if (isNaN(completed)) return;
                          await fetch('/api/admin/capacity/progress', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ date: row.date, completedUnits: completed }),
                          });
